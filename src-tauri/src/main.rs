...
fn image_enhance(image_data_base64: &str, amt: i16) -> Result<String, String> {
    ...
    // for pixel in img.get_raw_pixels() {
    //     buffer.push(pixel);
    // }
    // let raw_pixels = img.get_raw_pixels();
    // let enhanced_image = encode(&buffer);
    // enhanced_image
    // Ok(enhanced_image)
    Ok(img.get_base64())
}
...
fn segmenta(x: &PhotonImage, t: u8) -> PhotonImage {
    let (width, height) = (x.get_width(), x.get_height());
    // let img_vec = Vec::<u8>::new();
    let x_pixels = x.get_raw_pixels();
    let mut y_pixels = x_pixels.clone();

    for i in 0..width {
        for j in 0..height {
            let pixel = x_pixels[(i * height + j) as usize];
            if pixel > t {
                y_pixels[(i * height + j) as usize] = 255
            }
        }
    }
    let y = PhotonImage::new(y_pixels, width, height);
    y
}

#[tauri::command(rename_all = "snake_case")]
fn segment_image(image_data_base64: String, threshold: u8) -> Result<String, String> {
    let img = PhotonImage::new_from_base64(&image_data_base64);
    let new_img = segmenta(&img, threshold);
    Ok(new_img.get_base64())
}
fn main() {
